...
# The REMOTE_DOWNLOAD_CONFIG is the mandatory second argument.
REMOTE_DOWNLOAD_CONFIG=${2:-}

if [[ -z "$REMOTE_DOWNLOAD_CONFIG" ]]
then
  echo "Required argument REMOTE_DOWNLOAD_CONFIG ('true'|'false') not set!" >&2
  exit 1
fi
...
create_config () {
  local cache_key=$1
  local remote_download_config=$2
  ...
  if [[ "$remote_download_config" == "true" ]]
  then
    echo "The REMOTE_DOWNLOAD_CONFIG option has been set."
    echo "build --config=remote_download_config" >> bazel/bazelrcs/cache.bazelrc
  fi
}
...
create_config "$CACHE_KEY" "$REMOTE_DOWNLOAD_CONFIG" "$BAZEL_REMOTE_PASSWORD"
